...
Write-Host "Failure expected here, since repository already exists:"
cargo run init -d test_repo
...
cargo run add new_file.txt
Write-Host "Failure expected here:"
cargo run commit "Empty commit"

# Level 3: Status, Heads, Cat, Log
cargo run status
cargo run heads
Write-Host "Failure expected here: need to acquire revision id. Functionality is fine"
cargo run cat existing_file.txt
Write-Host "Failure expected here: a directory shouldn't be read"
